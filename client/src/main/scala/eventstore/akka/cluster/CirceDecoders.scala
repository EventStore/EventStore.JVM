...
private[eventstore] object CirceDecoders {
  ...
  implicit val codecForUuid: Decoder[Uuid] =
    Decoder.decodeUUID
  ...
  implicit val decoderForMemberInfo: Decoder[MemberInfo] = Decoder.instance {
    c =>
      c.downField("httpEndPointIp").key
       .fold(c.as[MappingPreSeries20].map(_.toMemberInfo))(_ => c.as[MappingPostSeries20].map(_.toMemberInfo))
  }
  ...
  object MappingPreSeries20 {
    ...
    implicit final class MappingPreSeries20Ops(val m: MappingPreSeries20) extends AnyVal {
      def toMemberInfo: MemberInfo = MappingPreSeries20.toMemberInfo(m)
    }
  ...
  object MappingPostSeries20 {
    ...
    implicit final class MappingPostSeries20Ops(val m: MappingPostSeries20) extends AnyVal {
      def toMemberInfo: MemberInfo = MappingPostSeries20.toMemberInfo(m)
    }
