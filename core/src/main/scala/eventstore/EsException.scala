...
@SerialVersionUID(1L)
final case class StreamNotFoundException(streamId: EventStream.Id) extends EsException(s"$streamId not found")
...
@SerialVersionUID(1L) final case class EventNotFoundException(streamId: EventStream.Id, number: EventNumber)
 ...
 */
@SerialVersionUID(1L) final case class OperationTimeoutException private[eventstore] (pack: PackOut)
