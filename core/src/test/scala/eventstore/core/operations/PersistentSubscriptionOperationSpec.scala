...
import operations.{PersistentSubscriptionOperation => PsO}
import NotHandled.{NotReady, TooBusy}
import PersistentSubscription.{Connect, Connected, EventAppeared}
import OnIncoming.{Continue, Ignore, Retry, Stop}
...
class PersistentSubscriptionOperationSpec extends OperationSpec {
      ...
      val o1 = operation.disconnected must beLike {
        case OnDisconnected.Continue(x) if x.version == 0 =>
          x.connected must beLike {
            case OnConnected.Retry(a, _) if a.version == 0 => ok
          }
      }
    }
  }
  ...
  "PersistentSubscriptionOperation when subscribed" should {
    ...
    "become connecting on disconnected" in new ConnectedScope {
      operation.disconnected must beLike {
        case OnDisconnected.Continue(x: PersistentSubscriptionOperation.Connecting[_]) if x.version == 1 => ok // Should be Connecting <= Persistent's version of subscribing.
      }
    }

    "become connected on connected and retry" in new ConnectedScope {
      operation.connected must beLike {
        case OnConnected.Retry(x: PersistentSubscriptionOperation.Connecting[_], `pack`) if x.version == 1 => ok
      }
    }
  ...
  private trait PsScope extends OperationScope {
    ...
    def packOut(msg: Out): PackOut = PackOut(msg, randomUuid)

    def resolvedEvent(streamId: EventStream.Id) = {
      val event = EventRecord(EventStream.Id(randomUuid.toString), EventNumber.Exact(10), TestData.eventData)
      ResolvedEvent(event, EventRecord(streamId, EventNumber.First, event.link(randomUuid)))
    }

    def eventAppeared(streamId: EventStream.Id): EventAppeared = {
      val event = EventRecord(streamId, EventNumber.First, TestData.eventData)
      eventAppeared(event)
    }
  ...
  private trait ConnectedScope extends PsScope {
    val connect     = Connect(streamId, groupName)
    val subTo       = subscribeTo(streamId)
    val pack        = packOut(subTo)
    val unsubscribe = PackOut(Unsubscribe, pack.correlationId, pack.credentials)
