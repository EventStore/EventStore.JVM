 ...
 */
case class Segment(markerByte: Byte, flags: Set[Any], correlationId: Uuid, auth: Option[AuthData], data: ByteString)

case class AuthData(login: String, password: String)

object Segment {

  //  [1 marker][1 flags][16 correlation id][16 login][16 password][...]  TODO


  val lengthHeaderLength = 4
  val lengthLimit = 64 * 1024 * 1024

  def apply(bs: ByteString): Segment = {
    val iterator = bs.iterator


    val markerByte = iterator.getByte
    val flagsByte = iterator.getByte
    val correlationIdBytes = iterator.take(16)
