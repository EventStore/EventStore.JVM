...
case class WriteEventsOperation(pack: PackOut, client: ActorRef, inFunc: InFunc, outFunc: Option[OutFunc]) extends Operation {
  ...
  def inspectIn(in: Try[In]) = {
    ...
    in match {
      case Success(_: WriteEventsCompleted)                            => succeed()
      ...
      case Failure(x @ EsException(EsError.WrongExpectedVersion, None)) =>
        ...
        val message = s"Write failed due to WrongExpectedVersion: ${writeEvents.streamId}, ${writeEvents.expectedVersion}"
