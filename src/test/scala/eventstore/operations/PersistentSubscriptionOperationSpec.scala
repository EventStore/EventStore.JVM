...
import eventstore.PersistentSubscription.{ Connect, Connected }
import eventstore.operations.{ PersistentSubscriptionOperation => PsO }
import eventstore._
import eventstore.operations.OnIncoming.{ Ignore, Continue, Retry, Stop }
...
class PersistentSubscriptionOperationSpec extends OperationSpec {
  "SubscriptionOperation when subscribing" should {
    ...
    "become subscribed on success" in new ConnectingScope {
      operation.inspectIn(Try(connected)) mustEqual Continue(
        SubscriptionOperation.Subscribed(connect, pack, client, ongoing = true, 1),
        Try(connected)
      )
    }
  ...
  "SubscriptionOperation when subscribed" should {
    ...
    "become subscribing on disconnected" in new ConnectedScope {
      operation.disconnected must beLike {
        case OnDisconnected.Continue(x: SubscriptionOperation.Subscribing) if x.version == 1 => ok
      }
    }

    "become subscribing on connected and retry" in new ConnectedScope {
  ...
  private trait ConnectingScope extends PsScope {
    ...
    lazy val connected = Connected(subscriptionId)
  }
  ...
  private trait ConnectedScope extends PsScope {
    ...
    val unsubscribe = PackOut(Unsubscribe, pack.correlationId, pack.credentials)
    val operation = PsO.Connected(subscribeTo, pack, client, ongoing = true, 0)
  }
